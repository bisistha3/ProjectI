...
function initGoalCalculator() {
  ...
  if (!weightInput || !activitySelect || !goalDisplay) return;

  function calculate() {
    const weight = parseFloat(weightInput.value) || 70;
    const activity = activitySelect.value;

    // Basic formula: weight * 30-40ml based on activity
    let multiplier = 30;
    if (activity.includes('Medium')) multiplier = 35;
    if (activity.includes('High')) multiplier = 40;

    const goalMl = weight * multiplier;
    const goalL = (goalMl / 1000).toFixed(1);
    goalDisplay.textContent = goalL + ' Liters / Day';
  }
